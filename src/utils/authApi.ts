...
/**
 * Response DTO interface matching backend MessageDto
 */
interface MessageDto {
  message: string;
  accessToken?: string; // Optional: returned by /refresh and /verify/v2
}
 ...
 */
export const verifyV2 = async (accessToken: string): Promise<MessageDto> => {
  try {
 ...
 */
export const refreshToken = async (dispatch: AppDispatch): Promise<MessageDto> => {
  try {
 ...
 * @returns MessageDto response from backend
 */
export const logout = async (dispatch: AppDispatch): Promise<MessageDto> => {
  try {
